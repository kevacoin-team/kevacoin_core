...
//! Convert enabled/size values to kevacoin -prune setting.
...
//! Get pruning enabled value to show in GUI from kevacoin -prune setting.
...
//! Get pruning size value to show in GUI from kevacoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayKevacoinUnit")) {
        settings.setValue("DisplayKevacoinUnit", QVariant::fromValue(KevacoinUnit::KVA));
    }
    QVariant unit = settings.value("DisplayKevacoinUnit");
    if (unit.canConvert<KevacoinUnit>()) {
        m_display_kevacoin_unit = unit.value<KevacoinUnit>();
    } else {
        m_display_kevacoin_unit = KevacoinUnit::KVA;
        settings.setValue("DisplayKevacoinUnit", QVariant::fromValue(m_display_kevacoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if kevacoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_kevacoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<KevacoinUnit>() == m_display_kevacoin_unit) return;
    m_display_kevacoin_unit = new_unit.value<KevacoinUnit>();
    QSettings settings;
    settings.setValue("DisplayKevacoinUnit", QVariant::fromValue(m_display_kevacoin_unit));
    Q_EMIT displayUnitChanged(m_display_kevacoin_unit);
}

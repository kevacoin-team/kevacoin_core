...
void KevacoinGUI::createActions()
{
    ...
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Kevacoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Kevacoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a kevacoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Kevacoin command-line options").arg(PACKAGE_NAME));
...
void KevacoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Kevacoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void KevacoinGUI::incomingTransaction(const QString& date, KevacoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    ...
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(KevacoinUnits::formatWithUnit(unit, amount, true));
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<KevacoinUnit> units = KevacoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const KevacoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, KevacoinUnits::longName(unit)));
    }
...
void UnitDisplayStatusBarControl::createContextMenu()
{
    ...
    for (const KevacoinUnit u : KevacoinUnits::availableUnits()) {
        menu->addAction(KevacoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(KevacoinUnit newUnits)
{
    setText(KevacoinUnits::longName(newUnits));
}

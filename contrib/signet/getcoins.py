...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing kevacoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='kevacoin-cli', help='kevacoin-cli command to use')
...
parser.add_argument('kevacoin_cli_args', nargs='*', help='Arguments to pass on to kevacoin-cli (default: -signet)')

args = parser.parse_args()

if args.kevacoin_cli_args == []:
    args.kevacoin_cli_args = ['-signet']


def kevacoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.kevacoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = kevacoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = kevacoin_cli(['getnewaddress', 'faucet', 'bech32'])

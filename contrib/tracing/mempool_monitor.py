...
def main(kevacoind_path):
    kevacoind_with_usdts = USDT(path=str(kevacoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    kevacoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    kevacoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    kevacoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    kevacoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[kevacoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/kevacoind")
        exit(1)

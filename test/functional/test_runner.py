...
`test/functional/test_framework/test_framework.py:KevacoinTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_kevacoin_cli.py --legacy-wallet',
    'interface_kevacoin_cli.py --descriptors',
...
def main():
    ...
    enable_kevacoind = config["components"].getboolean("ENABLE_KEVACOIND")

    if not enable_kevacoind:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, skipunit=False):
    ...
    # Warn if kevacoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "kevacoind"], stdout=subprocess.DEVNULL).returncode == 0:

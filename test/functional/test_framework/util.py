...
def assert_fee_amount(fee, tx_size, feerate_KVA_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_KVA_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s KVA too low! (Should be %s KVA)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_KVA_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s KVA too high! (Should be %s KVA)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting KVA values"""
...
def get_fee(tx_size, feerate_kva_kvb):
    """Calculate the fee in KVA given a feerate is KVA/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_kva_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  KVA
...
def wait_until_helper_internal(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None, timeout_factor=1.0):
    ...
    from `KevacoinTestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "kevacoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        if platform.system() == "Darwin":
            ...
            datadir = temp_dir / ".kevacoin"
    return env, datadir


def append_config(datadir, options):
    with open(os.path.join(datadir, "kevacoin.conf"), 'a', encoding='utf8') as f:

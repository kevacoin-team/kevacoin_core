...
TMPDIR_PREFIX = "kevacoin_func_test_"
...
class KevacoinTestMetaClass(type):
    """Metaclass for KevacoinTestFramework.

    Ensures that any attempt to register a subclass of `KevacoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'KevacoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("KevacoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("KevacoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class KevacoinTestFramework(metaclass=KevacoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave kevacoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop kevacoinds after the test execution")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "kevacoind": ("kevacoind", "KEVACOIND"),
            "kevacoin-cli": ("kevacoincli", "KEVACOINCLI"),
            "kevacoin-util": ("kevacoinutil", "KEVACOINUTIL"),
            "kevacoin-wallet": ("kevacoinwallet", "KEVACOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: kevacoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("KevacoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'kevacoind', self.options.kevacoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'kevacoin-cli', self.options.kevacoincli) for v in versions]
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the kevacoind
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as kevacoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    kevacoind=self.options.kevacoind,
                    kevacoin_cli=self.options.kevacoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in kevacoin.conf
    ...
    def skip_if_no_kevacoind_tracepoints(self):
        """Skip the running test if kevacoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("kevacoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_kevacoind_zmq(self):
        """Skip the running test if kevacoind has not been compiled with zmq support."""
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if kevacoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("kevacoin-wallet has not been compiled")

    def skip_if_no_kevacoin_util(self):
        """Skip the running test if kevacoin-util has not been compiled."""
        if not self.is_kevacoin_util_compiled():
            raise SkipTest("kevacoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if kevacoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("kevacoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether kevacoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether kevacoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_kevacoin_util_compiled(self):
        """Checks whether kevacoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_KEVACOIN_UTIL")
